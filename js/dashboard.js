...
function computedStats(){
  var artists={},years=[],withId=0,withDesc=0;
  ...
  var decades={};
  years.forEach(function(y){var d=Math.floor(y/10)*10;decades[d]=(decades[d]||0)+1;});
  return {
    total:RECS.length,
    ...
    decades:Object.keys(decades).sort().map(function(d){return {d:d,n:decades[d]};}),
  ...
  document.getElementById("dashstats").innerHTML=
    ...
    (s.decades.length?"<div class='deccard'><div class='ktitle'>By decade</div>"+
      "<div class='decs'>"+s.decades.map(function(d){
        var max=Math.max.apply(null,s.decades.map(function(x){return x.n;}));
        return "<button class='dec' data-decade='"+d.d+"'><div class='decbar' style='height:"+
...
function openDecadeDive(dec){
  var lo=+dec,hi=lo+9,hits=[];
  RECS.forEach(function(r){
    var y=cachedYear(r);
    if(y&&/^\d{4}$/.test(y)&&+y>=lo&&+y<=hi)hits.push({r:r,y:+y});
  });
  ...
  el.innerHTML=
    "<div class='assess'>"+
      "<div class='divehead'><span class='ra'>"+String(lo).slice(2)+"s</span>"+
      ...
      (hits.length
        ? "<div class='ablock'><div class='ktitle'>Records</div><p class='aitem'>"+
          hits.map(function(h){
            return h.y+" \u2014 "+esc(h.r.a)+", "+esc(h.r.t);
          }).join("<br>")+"</p></div>"
...
document.getElementById("view-dash").addEventListener("click",function(e){
  ...
  if(dec){openDecadeDive(dec);return;}
